 ...
 *
 */
class AesRsaEncrypt private constructor(context: Context): IEncrypt {
    ...
    @Throws(Exception::class)
    override fun getCipher(key: String?, mode: Int): Cipher {
        val cipher = Cipher.getInstance(AES_GCM_NO_PADDING)
        cipher.init(mode, getAESKeySpec(key), ivParameterSpec)
        return cipher
    }
    ...
    private fun getAESKeySpec(key: String?): SecretKeySpec {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN_MR2) {
            val encryptedKey = SpUtils.getAESKey(mContext)
